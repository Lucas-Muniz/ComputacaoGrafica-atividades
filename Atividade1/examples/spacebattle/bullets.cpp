...
void Bullets::paintGL() {
  ...
  for (auto &bullet : m_bullets) {
    glUniform4fv(m_colorLoc, 1, &bullet.m_color.r);
...
void Bullets::update(Ship &ship, const GameData &gameData, float deltaTime) {
  ...
  if (gameData.m_input[static_cast<size_t>(Input::Fire)] &&
    ...
    if (ship.m_bulletCoolDownTimer.elapsed() > 250.0 / 1000.0 && !ship.m_fieldactive) {
  ...
  // Remove dead bullets
  m_bullets.erase(std::remove_if(m_bullets.begin(), m_bullets.end(), [](const Bullet &p) { return p.m_dead; }),
                m_bullets.end());
}
  ...
  if (gameData.m_state == State::Playing){
    if (enemies.m_bulletCoolDownTimer.elapsed() > 1.0) {
      ...
      for (auto &enemy : enemies.m_horde){
        if (!enemy.m_dead){
          // Bullets are shot in the direction of the enemy's forward vector
          glm::vec2 forward{glm::rotate(glm::vec2{0.0f, -1.0f}, enemy.m_rotation)};
          auto bulletSpeed{2.0f};

          Bullet bullet{.m_origin = Origin::Enemy,
                        .m_dead = false,
    ...
    // Remove dead bullets
    m_bullets.erase(
    std::remove_if(m_bullets.begin(), m_bullets.end(), [](const Bullet &p) { return p.m_dead; }),
    m_bullets.end());
}
